...
with g.as_default():
    ...
    with tf.device("/job:worker/task:%d" % FLAGS.task_index):
        ...
        _, serialized_example = reader.read(filename_queue, name = "serialized_example_%d" % FLAGS.task_index)
        ...
        label = tf.cast(features['label'], tf.float32, name = "label_%d" % FLAGS.task_index)
    ...
    with tf.device("/job:worker/task:0"):
        update_w = w.scatter_sub(tf.IndexedSlices(local_gradient_val, index.values))
    ...
    with tf.Session("grpc://vm-11-%d:2222" % (FLAGS.task_index+1)) as sess:
        ...
        for i in range(0, num_iterations):
            ...
            if ((i < 20 and (i % 5) == 0) or(i % 20) == 0) and FLAGS.task_index == 0:
